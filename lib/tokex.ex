defmodule Indodax do
  ...
  @doc """
  Get the last price for spesific pair. You can check `Indodax.summaries/0` to knwo all available pairs.


  ## Examples

      iex> Indodax.ticker("zec_btc")
      {:ok,
       %{
  ...
  @doc """
  Get the last trades by all user for spesific pair. You can check `Indodax.summaries/0` to knwo all available pairs.


  ## Examples

      iex> Indodax.trades("zec_btc")
      {:ok,
       [
         %{
           "amount" => "162.69629057",
  ...
  @doc """
  Get public order book by all user for spesific pair. You can check `Indodax.summaries/0` to knwo all available pairs.


  ## Examples

      iex> Indodax.depth("ten_btc")
        {:ok,
         %{
           "buy" => [
  ...
  Say you want to buy 1 ZEC at the price of 0.000200 BTC you can put order like this
      ...
      iex> Indodax.trade("zec_btc", "buy", 0.000200, [zec: 1])
      {:ok,
       %{
         "balance" => %{
           "appc" => "0.00000000",
  ...
  defp tapi_call(%{} = params) do
    apikey = Application.get_env(:indodax, :public)
    apisecret = Application.get_env(:indodax, :secret)
